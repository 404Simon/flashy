...
impl FlashcardService {
    ...
    pub async fn list_decks(
        ...
        let mut items = self
            .query_decks(actor, project_id, cursor, Some(limit + 1))
            .await?;
    ...
    async fn query_decks(
        ...
        let rows = sqlx::query_as::<_, DeckSummary>(r#"SELECT fd.id, fd.project_id, fd.name, fd.description, fd.created_at,
            ...
            WHERE fd.project_id = ? AND sp.user_id = ?
              AND (? IS NULL OR fd.created_at < ? OR (fd.created_at = ? AND fd.id < ?))
            GROUP BY fd.id ORDER BY fd.created_at DESC, fd.id DESC LIMIT COALESCE(?, -1)"#)
    ...
    pub async fn list_cards_all(
        ...
        self.query_cards(actor, deck_id, file_id, None, None).await
    }
    ...
    pub async fn list_cards(
        ...
        let mut items = self
            .query_cards(actor, deck_id, file_id, cursor, Some(limit + 1))
            .await?;
        let more = items.len() > limit as usize;
        items.truncate(limit as usize);
        let next_cursor = if more {
            items
                ...
                .map(|x| {
                    encode_cursor(&CardCursor {
                        deck_id,
                        file_id,
                        after_created_at: x.created_at.clone(),
                        after_id: x.id,
                    })
                })
                .transpose()?
    ...
    async fn query_cards(
        ...
        let rows = sqlx::query_as::<_, Flashcard>(r#"SELECT fc.id, fc.deck_id, fc.front, fc.back, fc.document_reference,
            ...
            WHERE fc.deck_id = ? AND sp.user_id = ? AND (? IS NULL OR fc.file_id = ?)
              AND (? IS NULL OR fc.created_at > ? OR (fc.created_at = ? AND fc.id > ?))
            ORDER BY fc.created_at ASC, fc.id ASC LIMIT COALESCE(?, -1)"#)
            .bind(actor.user_id()).bind(deck_id).bind(actor.user_id()).bind(file_id).bind(file_id)
            .bind(after.as_ref().map(|cursor| cursor.after_created_at.as_str()))
            .bind(after.as_ref().map(|cursor| cursor.after_created_at.as_str()))
            .bind(after.as_ref().map(|cursor| cursor.after_created_at.as_str()))
            .bind(after.as_ref().map(|cursor| cursor.after_id))
            .bind(limit).fetch_all(&self.pool).await?;
